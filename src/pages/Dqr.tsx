...
// スライダーの再レンダリングを抑制するためにメモ化
const ControlSlider = memo(({ label, value, min, max, step, onChange, icon: Icon }) => (
...
const App = () => {
  ...
  // --- ステート ---
  const [cardType, setCardType] = useState('unit');
  ...
  const getAutoCorrectionScale = () => {
    return cardType === 'unit' ? UNIT_SCALE : SKILL_SCALE;
  };
  ...
  // useCallbackで関数を固定し、Sliderの不要な再レンダリングを防ぐ
  const handleLayerChange = useCallback((layer, key, value) => {
    setLayers(prev => ({
      ...prev,
  ...
  const handleFileUpload = (e, layer) => {
  ...
  return (
    <div className="flex h-screen bg-[#0f1115] overflow-hidden font-sans text-white">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin">
          ...
          <div className="bg-gray-100/10 p-5 rounded-2xl border border-white/5 space-y-4">
            ...
            <div className="space-y-3">
              <div className={`grid ${cardType === 'unit' ? 'grid-cols-2' : 'grid-cols-1'} gap-3`}>
                ...
                {cardType === 'unit' && (
                  <div>
                    ...
                    <input name="tribe" maxLength={5} value={cardData.tribe} onChange={handleInputChange} className="w-full bg-gray-100/10 rounded-lg p-2 text-sm outline-none border border-white/5 focus:border-yellow-500/50 transition-all text-white" placeholder="冒険者など" />
                  </div>
                )}
              </div>
              ...
              <div>
                <label className="text-[10px] text-gray-500 block mb-1 font-bold">効果テキスト</label>
              ...
              <ControlSlider 
                ...
                onChange={(v) => setCardData(p => ({...p, fontSize: v}))} 
              />
            </div>
          </div>
