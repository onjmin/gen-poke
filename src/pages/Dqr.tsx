...
const ControlSlider = memo(
	({ label, value, min, max, step, onChange, icon: Icon }: ControlSliderProps) => (
...
const Dqr = () => {
	...
	const parseDescription = (text: string) => {
		...
		return parts.map((part, i) => {
			if (part.startsWith("[") && part.endsWith("]")) {
				return (
					<span key={i} style={{ color: "#f8bb44" }} className="font-bold">
						{part.slice(1, -1)}
					</span>
				);
			}
	...
	// 【修正】窓（くり抜き部分）のサイズを調整：さらに深く（フチを厚く）、位置を数ピクセル下へ
	const getWindowMaskPath = () => {
		const W = CARD_WIDTH; // 380px想定
		const H = CARD_HEIGHT;

		// 左右マージンをさらに深く (40 -> 55)
		// 上(y)を数px下げ、高さ(h)も調整して下端を数px下げる
		const win =
			cardType === "unit"
				? { x: 55, y: 20, w: 270, h: 225 } // 上端15->20、下端も伸びて225
				: { x: 60, y: 30, w: 260, h: 220 }; // 特技側も同様に深く・低く

		// evenoddルールでくり抜きを表現
		return `M 0 0 H ${W} V ${H} H 0 Z M ${win.x} ${win.y} h ${win.w} v ${win.h} h -${win.w} Z`;
	};
		...
		<div className="flex flex-wrap md:flex-nowrap min-h-[100dvh] md:h-full bg-[#0f1115] overflow-auto font-sans text-white rounded-lg">
			...
			<div className="w-full md:flex-1 flex flex-col items-center justify-center p-6 bg-black/20 overflow-auto">
				...
				>
					...
					{/* レイヤー3: 目隠しレイヤー (真ん中が四角くくり抜かれた赤い板) */}
					<div
						className="absolute inset-0 z-[15] pointer-events-none"
						style={{
							backgroundColor: "#000000",
							clipPath: `path(evenodd, "${getWindowMaskPath()}")`,
						}}
					/>
					...
					<img
						...
						style={{
							...
							width: CARD_WIDTH,
							height: CARD_HEIGHT,
							transform: `scale(${FRAME_SCALES[cardType]})`,
							transformOrigin: "top left",
						}}
					/>

					<div className="absolute top-[-1px] left-[2px] w-[85px] h-[85px] z-30 flex items-center justify-center">
						<span className="text-white text-[58px] font-black tracking-tighter stat-font black-outline">
							{cardData.cost}
						</span>
					</div>

					<div className="absolute left-0 w-full z-30 flex justify-center top-[241px]">
						<span className="text-white text-[20px] font-bold tracking-tighter name-outline">
							{cardData.name}
						</span>
					</div>

					<div
						className={`absolute left-1/2 -translate-x-1/2 w-[290px] z-30 flex items-center justify-center px-4 text-center ${cardType === "unit" ? "bottom-[78px] h-[110px]" : "bottom-[70px] h-[125px]"}`}
					>
						<div
							className="font-bold leading-[1.3] w-full"
							style={{
					...
					{cardType === "unit" && (
						<>
							<div className="absolute left-0 w-full z-30 flex justify-center bottom-[31px]">
								<span className="text-white text-[14px] font-bold tracking-tight tribe-outline">
									{cardData.tribe}
								</span>
							</div>
							<div className="absolute bottom-[7px] left-[5px] w-[75px] h-[75px] z-30 flex items-center justify-center">
								<span className="text-white text-[52px] font-black tracking-tighter stat-font black-outline">
									{cardData.attack}
								</span>
							</div>
			...
			<div className="w-full md:w-[450px] border-l border-white/5 bg-[#161920] flex flex-col h-full shadow-2xl">
				...
				<div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin">
					<div className="flex p-1 bg-gray-100/10 rounded-xl border border-white/5">
						<button
							onClick={() => setCardType("unit")}
							className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${cardType === "unit" ? "bg-yellow-600 shadow-lg" : "text-gray-500 hover:text-white"}`}
						>
							ユニット
						</button>
						<button
							onClick={() => setCardType("skill")}
							className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${cardType === "skill" ? "bg-yellow-600 shadow-lg" : "text-gray-500 hover:text-white"}`}
						>
							特技
						</button>
					</div>
					...
					<div className="bg-gray-100/10 p-5 rounded-2xl border border-white/5 space-y-4">
						<h2 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] flex items-center gap-2">
							<Type size={12} className="text-yellow-500" /> 文字情報
						</h2>
						<div className="space-y-3">
							<div className={`grid ${cardType === "unit" ? "grid-cols-2" : "grid-cols-1"} gap-3`}>
								<div>
									...
									<input
										...
										className="w-full bg-gray-100/10 rounded-lg p-2 text-sm outline-none border border-white/5 focus:border-yellow-500/50 transition-all text-white"
									/>
								</div>
								{cardType === "unit" && (
									<div>
							...
							<div className="grid grid-cols-3 gap-3">
								<div>
									...
									<input
										...
										value={cardData.cost}
										onChange={handleInputChange}
										className="w-full bg-gray-100/10 rounded-lg p-2 text-sm outline-none border border-white/5 text-white"
									/>
								</div>
								{cardType === "unit" && (
									<>
							...
							<textarea
								...
								onChange={handleInputChange}
								className="w-full bg-gray-100/10 rounded-lg p-3 text-sm outline-none resize-none leading-relaxed border border-white/5 text-white"
							/>
							<ControlSlider
								label="フォントサイズ"
								...
								max={30}
								step={1}
								onChange={(v) => setCardData((p) => ({ ...p, fontSize: parseInt(v) }))}
							/>
						</div>
					</div>
					...
					<div className="bg-gray-100/10 p-5 rounded-2xl border border-white/5 space-y-6">
						<h2 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] flex items-center gap-2">
							<ImageIcon size={12} className="text-blue-500" /> 画像調整
						</h2>
						{(["unitImg", "bgImg"] as const).map((layerKey) => (
							<div key={layerKey} className="space-y-3 p-3 bg-black/20 rounded-xl">
								<div className="flex items-center justify-between mb-1">
									<span className="text-[11px] font-bold text-gray-300">
										{layerKey === "unitImg" ? "イラスト" : "背景"}
									</span>
									<label className="cursor-pointer text-blue-400 text-[10px] hover:text-blue-300 font-black uppercase">
										UPLOAD
										<input
											type="file"
											className="hidden"
											accept="image/*"
											onChange={(e) => handleFileUpload(e, layerKey)}
										/>
									</label>
								</div>
								<div className="space-y-3">
									<ControlSlider
										icon={Move}
										label="X"
										value={layers[layerKey].x}
										min={-500}
										max={500}
										step={1}
										onChange={(v) => handleLayerChange(layerKey, "x", v)}
									/>
									<ControlSlider
										icon={Move}
										label="Y"
										value={layers[layerKey].y}
										min={-500}
										max={500}
										step={1}
										onChange={(v) => handleLayerChange(layerKey, "y", v)}
									/>
									<ControlSlider
										icon={Maximize}
										label="Scale"
										value={layers[layerKey].scale}
										min={0.1}
										max={5}
										step={0.01}
										onChange={(v) => handleLayerChange(layerKey, "scale", v)}
									/>
								</div>
							</div>
						))}
